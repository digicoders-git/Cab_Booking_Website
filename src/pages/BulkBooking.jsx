...
const BulkBooking = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('success') === 'true') {
      setSuccess(true);
      // Clean up URL
      window.history.replaceState({}, '', '/bulk-booking');
    } else if (params.get('error')) {
      setPaymentError(params.get('error').replace(/_/g, ' '));
      window.history.replaceState({}, '', '/bulk-booking');
    }
  }, [location]);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.walletDeducted) {
          ...
          window.location.href = data.paymentLinks.web;
        } else {
          setError("Payment link not generated by gateway.");
        }
      } else {
        setError(data.message || "Booking failed.");
      }
    } catch (err) {
      setError(err.message || "Booking failed. Please check your connection.");
    } finally {
      setLoading(false);
    }
  };
