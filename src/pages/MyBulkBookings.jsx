...
const MyBulkBookings = () => {
    ...
    const generateReceipt = async (booking) => {
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/bulk-bookings/receipt/${booking._id}`, {
                headers: {
            ...
            a.href = url;
            a.download = `Invoice_${booking._id.toString().slice(-6)}.pdf`;
            a.click();
            window.URL.revokeObjectURL(url);
        } catch (err) {
            console.error("Receipt error:", err);
            alert("Something went wrong while downloading the receipt.");
        }
    };
